...
export class AutocompleteObject {
  constructor(
      public placeholder: string, // Autocomplete placeholder
      ...
      public items: AutocompleteItem[], // The list of items
      public maxHeight?: number, // One item is 55px, maxHeight = 55 x maxResults
      public value?: string, // Set a default text
...
export class AutocompleteItem {
  constructor(
      public name: string, // Name to display
      public item: any // Object to send and get back once clicked on
  ) { }
}
...
export class AutocompleteComponent implements OnInit, OnDestroy {
  ...
  private selectItem(direction: string) {
    this.filteredItems.first().subscribe(((items: AutocompleteItem[]) => {
  ...
  private upKeyEntered(index: number, items: AutocompleteItem[]) {
    if (index > 0) {
      ...
      this.itemListContainer.nativeElement.scrollTop -= Constants.ITEM_HEIGHT;
    } else {
